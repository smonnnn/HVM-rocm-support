fn main() {
  ...
  // Builds hvm.hip
  if std::process::Command::new("hipcc").arg("--version").stdout(std::process::Stdio::null()).stderr(std::process::Stdio::null()).status().is_ok() {
    ...
    println!("cargo:warning=\x1b[1m\x1b[31mWARNING: hipcc compiler not found.\x1b[0m \x1b[1mHVM will not be able to run on GPU with rocm.\x1b[0m");
  }
/*
  // Builds hvm.cu
